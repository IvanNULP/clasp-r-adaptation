...
FINAL_FEATURES = FEATURES_CONTINUOUS + FEATURES_DISCRETE  # порядок фіксований
...
def empirical_cdf(x_sorted, values):
    """
    Повертає u ∈ [0,1] — позицію values у emp. CDF, побудованому
    на x_sorted (відсортований source-масив), через лінійну
    інтерполяцію рангів (searchsorted).
    """
    n = len(x_sorted)
    ranks = np.searchsorted(x_sorted, values, side="left")
    u = np.clip(ranks / n, 1.0/(2*n), 1 - 1.0/(2*n))  # уникнення 0/1
    return u

def empirical_quantile(x_sorted, u):
    """Inverse CDF (quantile function) з відсортованого target-масиву."""
    n = len(x_sorted)
...
def quantile_transport(X_source, X_target):
    """
    Явний, детермінований per-feature quantile mapping:
      x' = F_target^{-1}( F_source(x) )
    Застосовується ТІЛЬКИ до continuous features (CONT_IDX).
    Discrete features (protocol, dst_port) залишаються untouched.
    """
    X_adapted = X_source.copy()
    for j in CONT_IDX:
        x_s_sorted = np.sort(X_source[:, j])
        x_t_sorted = np.sort(X_target[:, j])
        u = empirical_cdf(x_s_sorted, X_source[:, j])
        X_adapted[:, j] = empirical_quantile(x_t_sorted, u)
...
print("="*90)
print("CLASP-R — Step 4A: Quantile Transport Benchmark")
print(f"Continuous features (QT applied): {FEATURES_CONTINUOUS}")
print(f"Discrete features (untouched):    {FEATURES_DISCRETE}")
print("="*90)

results_path = RES_DAT / "v10_quantile_transport_240.csv"
...
for tgt_ds in FLOW_DATASETS:
    ...
    for src_ds in FLOW_DATASETS:
        ...
        for _, row in pair_manifest.iterrows():
            ...
            X_qt_all = quantile_transport(X_foreign, X_tgt_ref)
            ...
            for mn, model, classes in [("MLP", mlp_model, mlp_classes),
                ...
                probs_qt_all = predict_proba_aligned(
                    model, mn, X_qt_all, classes, mlp_classes)
                ...
                weights, applied = unsupervised_bbse(
                    probs_qt_all, frozen_entry, stability_action)
                probs_qt_bbse_all = probs_qt_all * weights[np.newaxis, :]

                preds_base    = probs_base_all[eval_mask].argmax(1)
                preds_qt      = probs_qt_all[eval_mask].argmax(1)
                preds_qt_bbse = probs_qt_bbse_all[eval_mask].argmax(1)

                rec[f"{mn}_BASE_error"] = compute_macro_error(preds_base, y_mapped, len(mlp_classes))
                rec[f"{mn}_QT_error"] = compute_macro_error(preds_qt, y_mapped, len(mlp_classes))
                rec[f"{mn}_QT_BBSE_error"] = compute_macro_error(preds_qt_bbse, y_mapped, len(mlp_classes))
...
print(f"\n✓ Step 4A (Quantile Transport) завершено: {len(all_records)} instances")
print(f"Saved: {results_path}")

qt_df = pd.DataFrame(all_records)
v9_df = pd.read_csv(RES_DAT/"v9_operational_adaptation_eval_240.csv")

print(f"\n{'='*90}")
print("ПОПЕРЕДНЄ ПОРІВНЯННЯ: Global CORAL (v9) vs Quantile Transport")
print(f"{'='*90}")
for mn in ["MLP","RF","XGB"]:
    base = v9_df[f"{mn}_BASE_error"].mean()
    coral = v9_df[f"{mn}_CORAL_error"].mean()
    clasp_coral = v9_df[f"{mn}_CLASP_error"].mean()
    qt = qt_df[f"{mn}_QT_error"].mean()
    qt_bbse = qt_df[f"{mn}_QT_BBSE_error"].mean()

    print(f"\n{mn}:")
    print(f"  BASE:                          {base:.4f}")
    print(f"  Global CORAL:                  {coral:.4f}  (Δ={coral-base:+.4f})")
    print(f"  Global CORAL + BBSE:           {clasp_coral:.4f}  (Δ={clasp_coral-base:+.4f})")
    print(f"  Quantile Transport:            {qt:.4f}  (Δ={qt-base:+.4f})")
    print(f"  Quantile Transport + BBSE:     {qt_bbse:.4f}  (Δ={qt_bbse-base:+.4f})")